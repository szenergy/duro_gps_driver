...
class DuroGps : public rclcpp::Node
{
public:
  DuroGps() : Node("duro_node")
  {

    RCLCPP_INFO_STREAM(this->get_logger(), "duro_node started");
    navsatfix_pub = this->create_publisher<sensor_msgs::msg::NavSatFix>("navsatfix", 100);
    odom_pub = this->create_publisher<nav_msgs::msg::Odometry>("odom", 100);
    imu_pub = this->create_publisher<sensor_msgs::msg::Imu>("imu", 100);
    mag_pub = this->create_publisher<sensor_msgs::msg::MagneticField>("mag", 100);
    euler_pub = this->create_publisher<geometry_msgs::msg::Vector3>("rollpitchyaw", 100);
    euler_pub_fake = this->create_publisher<geometry_msgs::msg::Vector3>("rollpitchyaw_fake", 100);
    pose_pub = this->create_publisher<geometry_msgs::msg::PoseStamped>("current_pose", 100);
    fake_pub = this->create_publisher<geometry_msgs::msg::PoseStamped>("current_pose_fake_orientation", 100);
    status_flag_pub = this->create_publisher<std_msgs::msg::UInt8>("status_flag", 100);
    status_string_pub = this->create_publisher<std_msgs::msg::String>("status_string", 100);
    time_ref_pub = this->create_publisher<sensor_msgs::msg::TimeReference>("time_ref", 100);
    tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);

    this->declare_parameter<std::string>("ip_address", "192.168.0.222");
    ...
    this->declare_parameter<std::string>("tf_child_frame_id", "gps");

    this->get_parameter("ip_address", tcp_ip_addr);
    this->get_parameter("port", tcp_ip_port);
    this->get_parameter("gps_receiver_frame", gps_receiver_frame);
    this->get_parameter("imu_frame", imu_frame);
    this->get_parameter("utm_frame", utm_frame);
    this->get_parameter("orientation_source", orientation_source);
    this->get_parameter("z_coord_ref_switch", z_coord_ref_switch);
    this->get_parameter("z_coord_exact_height", z_coord_exact_height);
    this->get_parameter("euler_based_orientation", euler_based_orientation);
    this->get_parameter("tf_frame_id", tf_frame_id);
    this->get_parameter("tf_child_frame_id", tf_child_frame_id);

    RCLCPP_INFO(this->get_logger(), "Starting GPS Duro...");
    RCLCPP_INFO(this->get_logger(), "Connecting to duro on %s:%d", tcp_ip_addr.c_str(), tcp_ip_port);

    if (z_coord_ref_switch.compare("exact") == 0)
    {
      RCLCPP_INFO_STREAM(this->get_logger(), "Exact height (z): " << z_coord_exact_height);
    }
    RCLCPP_INFO_STREAM(this->get_logger(), "TF child frame id: " << tf_child_frame_id);

    setup_socket();
    sbp_state_init(&sbp_state);
    sbp_register_callback(&sbp_state, SBP_MSG_POS_LLH, pos_ll_callback_static, this, &pos_ll_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_ORIENT_QUAT, orientation_callback_static, this, &orientation_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_ORIENT_EULER, orientation_euler_callback_static, NULL, &orientation_euler_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_IMU_RAW, imu_callback_static, NULL, &imu_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_IMU_AUX, imu_aux_callback_static, NULL, &imu_aux_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_MAG_RAW, mag_callback_static, NULL, &mag_callback_node);
    sbp_register_callback(&sbp_state, SBP_MSG_GPS_TIME, time_callback_static, NULL, &time_callback_node);

    RCLCPP_INFO(this->get_logger(), "Success on %s:%d", tcp_ip_addr.c_str(), tcp_ip_port);
    // Call on_timer function every 100 milliseconds
    timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&DuroGps::timer, this));
  }

private:
  static void pos_ll_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->pos_ll_callback(sender_id, len, msg, context);
  }
  static void orientation_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->orientation_callback(sender_id, len, msg, context);
  }
  static void orientation_euler_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->orientation_euler_callback(sender_id, len, msg, context);
  }
  static void imu_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->imu_callback(sender_id, len, msg, context);
  }
  static void imu_aux_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->imu_aux_callback(sender_id, len, msg, context);
  }
  static void mag_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->mag_callback(sender_id, len, msg, context);
  }
  static void time_callback_static(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    DuroGps *instance = static_cast<DuroGps *>(context);
    instance->time_callback(sender_id, len, msg, context);
  }
  ...
  void time_callback(u16 sender_id, u8 len, u8 msg[], void *context)
  {
    ...
    time_ref_msg.header.stamp = this->now();
  ...
  rclcpp::TimerBase::SharedPtr timer_{nullptr}; // Timer for publishing
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<DuroGps>());
  rclcpp::shutdown();
  close_socket();
  return 0;
}